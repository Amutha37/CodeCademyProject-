  ...
  it("", function () {
    ...
    let addEventListenerCallbacks = {
      "$event, $handler": function (event, handler) {
        ...
        if (handler.name !== "sharePhoto") {
          return {
            failure:
              "Are you setting `sharePhoto` as your second argument of `addEventListener()`?",
          };
        }
        return true;
      },
    ...
    let onEventCallbacks = {
      "$event, $handler": function (event, handler) {
        ...
        if (handler.name !== "sharePhoto") {
          return {
            failure:
              "Are you setting `sharePhoto` as your event handler function?",
          };
        }
        return true;
      },
    ...
    assert.isOk(
      ...
      varCallbacks.failure ||
        "Did you add an event handler for `share` element?"
    );
  });
});
