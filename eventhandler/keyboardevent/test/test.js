...
code = code
  .splice(learnerCodeStart, code.length - (learnerCodeStart - 1))
  .join("\n");
  ...
  it("", function () {
      ...
      $eventMethod: function (eventMethod) {
        if (eventMethod.name === "addEventListener") {
          ...
          let isMatchThree = Structured.match(code, structureThree, {
            varCallbacks: addEventListenerCallbacks,
          });

          assert.isOk(
            isMatchTwo,
            "Is your first argument an event type and the second argument event handler function?"
          );
          assert.isOk(
            isMatchThree,
            addEventListenerCallbacks.failure ||
              "Are you giving an event handler function as the second argument?"
          );
          return true;
        }
        ...
        if (eventMethod.name.includes("on")) {
          let isMatchFour = Structured.match(code, structureFour, {
            varCallbacks: onEventCallbacks,
          });
          assert.isOk(
            isMatchFour,
            onEventCallbacks.failure ||
              "Did you set an event handler function for the `.onevent` property?"
          );
        ...
        return {
          failure:
            "Are you using either `.addEventListener()` or `.onevent` to add an event handler?",
        };
      },
    };
    ...
    let addEventListenerCallbacks = {
      "$event, $handler": function (event, handler) {
        ...
        if (handler.name !== "down") {
          return {
            failure:
              "Are you setting `down` as your second argument of `.addEventListener()`?",
          };
        }
        return true;
      },
      ...
      "$event, $handler": function (event, handler) {
        if (event.name !== "onkeyup") {
          if (event.name === "innerHTML") {
            return {
              failure:
                "Are you setting the event handler function as value of `.onevent` property?",
            };
          }
          return {
            failure:
              "Are you using the `.onkeyup` property to add your event handler?",
          };
        }

        if (handler.name !== "down") {
          return {
            failure: "Are you setting `down` as your event handler function?",
          };
        }
        return true;
      },
    ...
    assert.isOk(
      ...
      varCallbacks.failure ||
        "Did you add an event handler for the `document` element?"
    );
  });
});
