  ...
  it("", function () {
    ...
    let addEventListenerCallbacks = {
      "$event, $handler": function (event, handler) {
        ...
        if (handler.name !== "showItem") {
          return {
            failure:
              "Are you setting `showItem` as your second argument of `addEventListener()`?",
          };
        }
        return true;
      },
    ...
    let onEventCallbacks = {
      "$event, $handler": function (event, handler) {
        if (event.name !== "onmousedown") {
          ...
          return {
            failure:
              "Are you using the `onmousedown` property to add your event handler?",
          };
        }
        ...
        if (handler.name !== "showItem") {
          return {
            failure:
              "Are you setting `showItem` as your event handler function?",
          };
        }
        return true;
      },
    ...
    assert.isOk(
      ...
      varCallbacks.failure ||
        "Did you add an event handler for `itemFour` element?"
    );
  });
});
