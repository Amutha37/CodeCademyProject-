  ...
  it("", function () {
    ...
    let addEventListenerCallbacks = {
      "$event, $handler": function (event, handler) {
        ...
        if (handler.name !== "textReturn") {
          return {
            failure:
              "Are you setting `textReturn` as your second argument of `addEventListener()`?",
          };
        }
        return true;
      },
    ...
    let onEventCallbacks = {
      "$event, $handler": function (event, handler) {
        if (event.name !== "onclick") {
          ...
          return {
            failure:
              "Are you using the `onclick` property to add your event handler?",
          };
        }
        ...
        if (handler.name !== "textReturn") {
          return {
            failure:
              "Are you setting `textReturn` as your event handler function?",
          };
        }
        return true;
      },
    ...
    assert.isOk(
      ...
      varCallbacks.failure ||
        "Did you add an event handler for `close` element?"
    );
  });
});
