...
describe("", function () {
  it("", function () {
    ...
    assert.isOk(
      isMatchOne,
      "Did you assign a value to the `innerHTML` property of `debateElement` after you initialized `debateElement`?"
    );
    assert.isOk(
      isMatchTwo,
      "Did you assign `debateElement`'s `innerHTML` property the value of `compiledHtml`?"
    );
  });
});
